...
실제 제원표 헤더 예시를 받아 확인한 실제 구조 (2025-09):
  ...
  2행 = 세부 항목 (예: 위치/라인/층/건설코드/설비대수 - UTILITY 공통 항목,
                   유량/성상명/압력/배관수량/배관재질/설비모듈 - 대분류별 반복 항목)
  3행~ = 데이터. 같은 건설코드를 쓰는 여러 행이 이어질 수 있고, 건설코드 셀은
         그룹의 첫 행에만 채워지고 그 아래 행들은 비어 있다(carry-down 관례).

즉 "대공정/건설코드/설비모듈"의 3개 분류축 중, 대공정만 업로드 시 지정하는
메타데이터이고 건설코드/설비모듈은 시트 안의 데이터에서 직접 읽어야 한다.
설비모듈은 대분류(UTILITY 대분류)마다 각각 별도 열로 존재한다(GAS·AIR용, POWER용 ...).
...
DEFAULT_COMMON_FIELD_NAMES = ("위치", "라인", "층", "건설코드", "설비대수")
