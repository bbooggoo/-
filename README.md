...
- **2행 = 세부 항목** — UTILITY 대분류 아래 공통 항목(위치/라인/층/건설코드/설비대수)과, 대분류마다 반복되는
...
즉 대공정/건설코드/설비모듈 3개 분류축 중 **대공정만 업로드 시 지정하는 메타데이터**이고,
**건설코드와 설비모듈은 시트 데이터에서 직접 읽습니다.** 특히 설비모듈은 대분류마다 별도
열로 존재하므로(GAS·AIR용 설비모듈, POWER용 설비모듈 ...) 제원표 전체의 단일 값이 아닙니다.
...
- 각 열의 field_name은 `대분류_세부항목` 형태로 조합됩니다 (예: `GAS/AIR_유량`, `UPW_성상명`).
  공통 항목(위치/라인/층/건설코드/설비대수)은 대분류 접두어 없이 그대로 사용합니다.
