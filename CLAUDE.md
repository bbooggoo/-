...
GCS(SPECIALITY GAS·폐액·CHEMICAL) 대분류의 성상명/자재명은 아래처럼 "성질"로도
분류된다. 이 분류는 SUMMARY 탭·그래프 만들기 탭에서 자재명/성상명별 집계와
별도로 성질별 집계를 보여주는 데 쓰인다. **이 표는 `demo/spec-qa-workflow.html`
안의 `GCS_NATURE_MAP` 상수와 항상 같은 내용을 담아야 하는 단일 진실 공급원(지침)
이며, 이 앱 하나만이 아니라 타 PJT·타 프로젝트에서도 그대로 재사용하고 새 항목이
생기면 여기에 먼저 누적한다.**

- **SPECIALITY GAS** (독가연성/가연성/독성/불활성 중 분류):
  - 가연성: SiH4
  - 독가연성: PH3, B2H6, AsH3, GeH4, COS
  - 독성: WF6, NF3, HCl, SiF4
  - 불활성: C2F6
- **폐액** (산성/알칼리성/유기성/슬러지 중 분류):
  - 산성: ACID WASTE
  - 알칼리성: ALKALI WASTE
  - 유기성: SOLVENT WASTE
  - 슬러지: SLUDGE
- **CHEMICAL** (ACID/ALKALI/기타 중 분류):
  - ACID: 황산, 불산, BOE
  - ALKALI: 암모니아수, TMAH, DEVELOPER
  - 기타: IPA, 과산화수소, PR, 아세톤

**갱신 규칙:**
- 새 성상명/자재명이 생기면(제원표 업로드·수기입력 등으로 제원 DB에 나타나면)
  앱이 자동으로 GCS 기술팀에게 분류를 물어보는 질의를 생성한다. GCS 기술팀의
  회신으로 분류가 정해지면, **이 표와 `GCS_NATURE_MAP` 상수를 같은 세션 안에서
  함께 갱신**하고(코드만 고치고 이 표를 빠뜨리지 않는다) 게시·커밋·푸시까지
  마친다 — "Artifact ↔ git 동기화" 절의 동기화 순서를 그대로 따른다.
- 타 PJT/타 프로젝트에서 이 맵핑리스트를 참고하거나 새 항목을 추가할 때도 이
  절을 갱신 대상으로 삼는다 — 프로젝트마다 각자 다른 분류 기준을 만들지 않는다.
- 분류를 정할 수 없는 신규 항목은 임의로 짐작해서 채우지 말고, GCS 기술팀의
  실제 회신을 받은 뒤에만 이 표에 추가한다.
